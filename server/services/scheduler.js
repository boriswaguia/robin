 ...
 *
 *  1. Reminder check  — runs every 5 minutes
 *     Fires Web Push for mail items whose reminderAt is now due.
 *     5-min granularity is fine: users set reminders in advance, not to-the-second.
 *     → 288 DB queries/day (vs 720 at 2 min)
 *
 *  2. Overdue check   — runs once per day at 09:00 server time
 *     Finds items where dueDate has passed and no action has been taken.
 *     Daily is sufficient because the trigger condition only changes once per day.
 *     → 1 DB query/day
 */
import prisma from './db.js';
import { sendPushToUser, isPushConfigured } from './push.js';
...
const OVERDUE_CHECK_HOUR   = 9;              // 09:00 server local time

export function startScheduler() {
  console.log('Notification scheduler started (reminders: every 5 min | overdue: daily at 09:00)');
  ...
  runReminders(); // run once immediately on startup
  setInterval(runReminders, REMINDER_INTERVAL_MS);

  // ── Overdue check: once per day at 09:00 ─────────────────────────────────
  scheduleDaily(OVERDUE_CHECK_HOUR, () =>
    fireOverdueNotifications().catch((err) =>
      console.error('Overdue scheduler error:', err.message)));
}

/**
 * Schedule a callback to run once per day at the given hour (local time).
 * Calculates the exact ms until the next occurrence and uses a self-rescheduling timeout.
 */
function scheduleDaily(hour, fn) {
  ...
  function schedule() {
    ...
    console.log(`Overdue check scheduled in ${Math.round(delay / 60000)} min`);
    setTimeout(() => { fn(); schedule(); }, delay);
  }
...
async function fireReminderNotifications() {
  ...
  for (const item of dueMail) {
    try {
      ...
      console.error(`Failed to send reminder push for mail ${item.id}:`, err.message);
    }
  }
...
// ── 2. Overdue items (due date passed, no action taken) ──────────────────────
...
async function fireOverdueNotifications() {
  ...
  for (const item of overdueMail) {
    try {
      ...
      console.error(`Failed to send overdue push for mail ${item.id}:`, err.message);
    }
  }
